 ...
 */
export function optimizeSessionQuery(options: SessionQueryOptions): OptimizedSessionQuery {
  ...
  if (!fromTimestamp) {
    ...
    logger.warn("Session query without time bounds - adding default 7-day window", {
  ...
  if (!fields || fields.length === 0) {
    ...
    logger.warn("Session query requesting all fields - consider reducing field selection", {
      sessionId: options.sessionId,
      projectId: options.projectId,
...
export function validateSessionQuery(options: SessionQueryOptions): {
  ...
  if (!options.fromTimestamp) {
    ...
    recommendations.push("Add fromTimestamp parameter to improve query performance by 50-70%");
  } else {
    const daysDiff = (Date.now() - options.fromTimestamp.getTime()) / (1000 * 60 * 60 * 24);
    if (daysDiff > 30) {
      warnings.push("Time range exceeds 30 days");
      recommendations.push("Consider reducing time range to improve performance");
    }
  }
  ...
  if (!options.fields || options.fields.includes("all")) {
    ...
    recommendations.push("Specify only needed fields (e.g., ['core']) to reduce data transfer");
  }
  
  // Check limit
  if (!options.limit || options.limit > 100) {
    warnings.push("Large or unlimited result set");
    recommendations.push("Use pagination with limit <= 100 for better performance");
  }
...
export function estimateSessionQueryPerformance(options: SessionQueryOptions): {
  ...
  } else if (options.fields.includes("observations") || options.fields.includes("scores")) {
    score -= 10;
